...
public class OrderDAO {
    ...
    public boolean addOrder(String name, String pizzaMap, String friedMap, String deliveryTime) throws IOException {
        ...
        Map<Product, Integer> pm = objectMapper.readValue(pizzaMap, Map.class);
        pm.entrySet().forEach(entry -> {
            takeAwayOrder.addProduct(new PizzaProduct(entry.getKey().getID()), entry.getValue());
        });
        Map<Product, Integer> fm = objectMapper.readValue(friedMap, Map.class);
        fm.entrySet().forEach(entry -> {
            takeAwayOrder.addProduct(new FriedProduct(entry.getKey().getID()), entry.getValue());
        });
    ...
    public boolean addOrder(String email, String name, String deliveryAddress, String phone, String pizzaMap, String friedMap, String deliveryTime) throws IOException {
        ...
        Map<Product, Integer> pm = objectMapper.readValue(pizzaMap, Map.class);
        pm.entrySet().forEach(entry -> {
            deliveryOrder.addProduct(new PizzaProduct(entry.getKey().getID()), entry.getValue());
        });
        Map<Product, Integer> fm = objectMapper.readValue(friedMap, Map.class);
        fm.entrySet().forEach(entry -> {
            deliveryOrder.addProduct(new FriedProduct(entry.getKey().getID()), entry.getValue());
        });
    ...
    public boolean addOrder(int table, int sitting, String pizzaMap, String friedMap) throws IOException {
        ...
        Map<Product, Integer> pm = objectMapper.readValue(pizzaMap, Map.class);
        pm.entrySet().forEach(entry -> {
            internalOrder.addProduct(new PizzaProduct(entry.getKey().getID()), entry.getValue());
        });
        Map<Product, Integer> fm = objectMapper.readValue(friedMap, Map.class);
        fm.entrySet().forEach(entry -> {
            internalOrder.addProduct(new FriedProduct(entry.getKey().getID()), entry.getValue());
        });
